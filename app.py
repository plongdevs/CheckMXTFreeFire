...
def spam_loop(uid, ip, port, packet, iv_ms, end_time, stop_event=None):
    while time.time() < end_time:
        if uid not in active_spams or (stop_event and stop_event.is_set()):
            break
        try:
            send_packet_tcp(ip, port, packet, timeout=5)
            if uid in active_spams: active_spams[uid]['ok'] += 1
        except:
            if uid in active_spams: active_spams[uid]['fail'] += 1
        if uid in active_spams: active_spams[uid]['sent'] += 1
...
def api():
    ...
    # ── GUEST → JWT ──
        ...
        uid_guest = d.get('uid_guest',''); pw = d.get('password','') 
    ...
    # ══════════════════════════════════════════════
        ...
        try:
            ...
            # Khởi tạo luồng ngầm
            thread = threading.Thread(target=spam_loop, args=(uid, online_ip, online_port, packet, iv, end_time, stop_event))
            thread.daemon = True
            ...
            # Lưu đồng bộ xuống file ngay lập tức
        ...
        # KIỂM TRA HỒI SINH LUỒNG TỪ CACHE NẾU SERVER TỪNG BỊ SẬP
        if uid not in active_spams:
            cache = load_spam_cache()
            c_item = cache.get(str(uid))
            if c_item and c_item.get('status') == 'running' and c_item.get('end_time', 0) > time.time():
                stop_event = threading.Event()
                
                # Giải mã packet từ chuỗi Hex (trong json) trở lại định dạng Bytes
                pkt_data = c_item.get('packet')
                if isinstance(pkt_data, str):
                    try: pkt_data = bytes.fromhex(pkt_data)
                    except: pass
                ...
                active_spams[uid] = {
                    ...
                    'packet': pkt_data,
                    'interval': c_item.get('interval')
                }
                
                # Hồi sinh lại luồng ngầm với các tham số cũ
                thread = threading.Thread(
                    target=spam_loop, 
                    args=(uid, c_item.get('ip'), c_item.get('port'), pkt_data, c_item.get('interval'), c_item.get('end_time'), stop_event)
                )
                thread.daemon = True
                thread.start()
        ...
        s = active_spams[uid]
        save_spam_cache(active_spams) # Cập nhật liên tục số lượng đã gửi vào cache
